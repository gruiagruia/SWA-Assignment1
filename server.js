...
const create_alerts = (predictions) => {
  const alerts = predictions
    .filter(generator.alertable)
    .map((prediction, i) => alert(alert_id + i, 3, prediction));
...
const update_alerts = (forecast, time) => {
  ...
  const { positive: updated_alerts, negative: cancelled_alerts } = partition(
    (t) => t.new_prediction && generator.alertable(t.new_prediction)
  )(match_alerts);
  const unalerted = forecast.filter((p) => !alerts.some((a) => a.matches(p)));

  cancelled_alerts.forEach(
    ({ idx }) => (alerts[idx] = alerts[idx].cancelled())
  );
      ...
      alerts
        ...
        .forEach((alert) =>
          [...wss.clients]
            .filter(
              (client) =>
                client.readyState === WebSocket.OPEN && client.subscribed
            )
...
wss.on("connection", (ws, req) => {
  ws.on("message", (message) => {
    switch (message.toString()) {
      ...
      default:
        console.error(
          `Incorrect message: '${message}' from ${req.socket.remoteAddress} (${req.socket.remoteFamily})`
        );
    }
  });
